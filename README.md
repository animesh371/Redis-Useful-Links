# Redis-Useful-Links
Useful links to get started in Redis

# Redis Data structures
* [Introduction to Redis Data Structures](https://redis.io/topics/data-types-intro)
* [Data Types](https://redislabs.com/ebook/part-1-getting-started/chapter-1-getting-to-know-redis/1-2-what-redis-data-structures-look-like/)
* [Introduction to Hashes](https://scalegrid.io/blog/introduction-to-redis-data-structures-hashes/)
* [Sets](https://scalegrid.io/blog/introduction-to-redis-data-structures-sets/)
* [HyperLogLogs in Redis](https://thoughtbot.com/blog/hyperloglogs-in-redis)
* [Sorted Sets](https://redislabs.com/ebook/part-1-getting-started/chapter-1-getting-to-know-redis/1-2-what-redis-data-structures-look-like/1-2-5-sorted-sets-in-redis/)

# Redis Sentinel 
* [Redis Sentinel - official documentation](https://redis.io/topics/sentinel)
* [Sentinel - High Availability](https://medium.com/@amila922/redis-sentinel-high-availability-everything-you-need-to-know-from-dev-to-prod-complete-guide-deb198e70ea6)
* [Introduction to Redis Sentinel](https://www.objectrocket.com/blog/how-to/introduction-to-redis-sentinel)
* [Redis-Sentinel- Cheatsheet](https://lzone.de/cheat-sheet/Redis%20Sentinel)

# Redis Cluster 
* [Redis Cluster - official](https://redis.io/topics/cluster-tutorial)
* [Cluster - specification](https://redis.io/topics/cluster-spec)
* [Configuring & Running Cluster](https://codeflex.co/configuring-redis-cluster-on-linux)
* [Redis Cluster - Video](https://www.youtube.com/watch?v=ScsWSeJoq2E)

# Redis Clients
* [Jedis](https://www.baeldung.com/jedis-java-redis-client-library)
* [redis-py](https://pypi.org/project/redis/)

# Useful Books
* [Redis in Action](https://www.goodreads.com/book/show/16033444-redis-in-action)
* [Redis Essentials](https://www.goodreads.com/book/show/30514076-mastering-redis)
